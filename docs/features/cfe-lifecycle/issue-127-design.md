...
`CFE_INTERCEPTOR_CONFLICT`. `ChangeAndValidate` использует маркеры `#Удаление/#КонецУдаления` и
`#Вставка/#КонецВставки`; drift исходного hash даёт `CFE_SOURCE_CHANGED`.
