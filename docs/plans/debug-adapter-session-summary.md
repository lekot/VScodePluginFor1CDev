...
- **Локальные переменные** — `evalLocalVariables` **отключён** (вызывает TCP reset / crash dbgs). Протестировано 8 вариантов XML (probe-скрипт `test/debug-probe/evalLocalVarsProbe.ts`):
  - Любой элемент в namespace `debugCalculations` (`calc:`) → **CRASH dbgs** (ECONNRESET, exit 1)
  - `xsi:type="calc:CalculationSourceDataStorage"` на `<rdbg:expr>` → **CRASH**
  - Пустой `<rdbg:expr/>` → **CRASH**
  - Элементы в других namespace (rdbg, default, unqualified) → **HTTP 400** XDTO (не crash)
  - Без `<rdbg:expr>` вообще → **HTTP 400**
  - **Вывод:** баг платформы 8.3.27.1859 — XDTO-процессор крашится при обработке типа `CalculationSourceDataStorage`.
- **Evaluate (watch)** — не тестировалось. Конфигуратор использует `evalExpr` для показа переменных (подтверждено: «Выражение: a=0» в панели). Возможно `evalExpr` работает и обходит проблему `evalLocalVariables`.
- **Popup «Неизвестный модуль»** — при возврате из вложенной процедуры. Косметический баг.
...
1. **Попробовать `evalExpr` вместо `evalLocalVariables`** (приоритет):
   - Конфигуратор использует `evalExpr` для показа переменных (подтверждено скриншотом).
   - `evalExpr` может обходить баг `CalculationSourceDataStorage` — проверить в probe-скрипте.
   - Если `evalExpr` работает — реализовать Locals через серию `evalExpr` вызовов.
2. **Перехват трафика Конфигуратора** — нужен Fiddler/Wireshark (npcap) для loopback. `pktmon` не ловит 127.0.0.1. TCP-прокси работает технически, но Конфигуратор не подключается через HTTP-прокси, а TCP-протокол не даёт выбирать порт.
3. **evalLocalVariables** — баг платформы 8.3.27.1859, namespace `debugCalculations` крашит dbgs. Отложено до новой версии платформы или обходного пути.
4. **moduleIdResolver**: шаблоны для RecordSetModule и прочих `Ext/*.bsl`.
5. **Popup «Неизвестный модуль»** — косметический баг при возврате из вложенной процедуры.
