...
suite('MetadataParser edge branches', () => {
  ...
  test('parseTypeIndex reuses disk cache for unchanged type folders', async () => {
    const configPath = await fs.promises.mkdtemp(path.join(os.tmpdir(), '1cviewer-type-index-cache-cfg-'));
    const storagePath = await fs.promises.mkdtemp(path.join(os.tmpdir(), '1cviewer-type-index-cache-store-'));
    await fs.promises.mkdir(path.join(configPath, 'Catalogs'), { recursive: true });
    await fs.promises.writeFile(path.join(configPath, 'Catalogs', 'Goods.xml'), '<MetaDataObject/>', 'utf-8');
    ...
    try {
      ...
      const first = await MetadataParser.parseTypeIndex(configPath, 'Catalogs');
      const second = await MetadataParser.parseTypeIndex(configPath, 'Catalogs');

      assert.strictEqual(first[0].id, 'Catalogs.Goods');
      assert.strictEqual(second[0].id, 'Catalogs.Goods');
      assert.strictEqual(parseCount, 1, 'second parseTypeIndex call must load from disk cache without reparsing');
    } finally {
      MetadataParser.setTypeContentsCacheStoragePath(null);
      (FormatDetector.detect as unknown as typeof FormatDetector.detect) = originalDetect;
      (DesignerParser.parseTypeIndex as unknown as typeof DesignerParser.parseTypeIndex) = originalDesigner;
  ...
  test('parseTypeIndex respects bypassCache option', async () => {
    const configPath = await fs.promises.mkdtemp(path.join(os.tmpdir(), '1cviewer-type-index-bypass-cfg-'));
    const storagePath = await fs.promises.mkdtemp(path.join(os.tmpdir(), '1cviewer-type-index-bypass-store-'));
    await fs.promises.mkdir(path.join(configPath, 'Catalogs'), { recursive: true });
    await fs.promises.writeFile(path.join(configPath, 'Catalogs', 'Goods.xml'), '<MetaDataObject/>', 'utf-8');
    ...
    try {
      ...
      await MetadataParser.parseTypeIndex(configPath, 'Catalogs');
      assert.strictEqual(parseCount, 1);

      await MetadataParser.parseTypeIndex(configPath, 'Catalogs', { bypassCache: true });
      assert.strictEqual(parseCount, 2, 'bypassCache: true must bypass disk cache');
    } finally {
      MetadataParser.setTypeContentsCacheStoragePath(null);
      (FormatDetector.detect as unknown as typeof FormatDetector.detect) = originalDetect;
      (DesignerParser.parseTypeIndex as unknown as typeof DesignerParser.parseTypeIndex) = originalDesigner;
  ...
  test('parseTypeIndex invalidates disk cache when type folder signature changes', async () => {
    const configPath = await fs.promises.mkdtemp(path.join(os.tmpdir(), '1cviewer-type-index-inval-cfg-'));
    const storagePath = await fs.promises.mkdtemp(path.join(os.tmpdir(), '1cviewer-type-index-inval-store-'));
    await fs.promises.mkdir(path.join(configPath, 'ExchangePlans'), { recursive: true });
    await fs.promises.writeFile(path.join(configPath, 'ExchangePlans', 'Sync.xml'), '<MetaDataObject/>', 'utf-8');
    ...
    try {
      MetadataParser.setTypeContentsCacheStoragePath(storagePath);
      (FormatDetector.detect as unknown as (p: string) => Promise<ConfigFormat>) = async () => ConfigFormat.Designer;
      (DesignerParser.parseTypeIndex as unknown as (a: string, b: string) => Promise<TreeNode[]>) = async (_cfg, _type) => {
        parseCount += 1;
        return [{ id: 'ExchangePlans.Sync', name: 'Sync', type: MetadataType.ExchangePlan, properties: {} }];
      };

      await MetadataParser.parseTypeIndex(configPath, 'ExchangePlans');
      assert.strictEqual(parseCount, 1);

      // Add a new file to change signature
      await new Promise<void>((resolve) => setTimeout(resolve, 50));
      await fs.promises.writeFile(path.join(configPath, 'ExchangePlans', 'NewPlan.xml'), '<MetaDataObject/>', 'utf-8');

      await MetadataParser.parseTypeIndex(configPath, 'ExchangePlans');
      assert.strictEqual(parseCount, 2, 'signature change must trigger reparse and update cache');
    } finally {
      MetadataParser.setTypeContentsCacheStoragePath(null);
      (FormatDetector.detect as unknown as typeof FormatDetector.detect) = originalDetect;
      (DesignerParser.parseTypeIndex as unknown as typeof DesignerParser.parseTypeIndex) = originalDesigner;
  ...
  test('parseTypeIndex caches EDT format folders with Russian names and isolates cloned mutations', async () => {
    const configPath = await fs.promises.mkdtemp(path.join(os.tmpdir(), '1cviewer-type-index-edt-cfg-'));
    const storagePath = await fs.promises.mkdtemp(path.join(os.tmpdir(), '1cviewer-type-index-edt-store-'));
    const docPath = path.join(configPath, 'src', 'Documents', 'РеализацияТоваров');
    await fs.promises.mkdir(docPath, { recursive: true });
    await fs.promises.writeFile(path.join(docPath, 'РеализацияТоваров.mdo'), '<mdclass:Document/>', 'utf-8');

    const originalDetect = FormatDetector.detect;
    const originalEdt = EdtParser.parseTypeIndex;
    let parseCount = 0;
    try {
      MetadataParser.setTypeContentsCacheStoragePath(storagePath);
      (FormatDetector.detect as unknown as (p: string) => Promise<ConfigFormat>) = async () => ConfigFormat.EDT;
      (EdtParser.parseTypeIndex as unknown as (a: string, b: string) => Promise<TreeNode[]>) = async (_cfg, _type) => {
        parseCount += 1;
        return [
          {
            id: 'Documents.РеализацияТоваров',
      ...
      const first = await MetadataParser.parseTypeIndex(configPath, 'Documents');
      assert.strictEqual(first.length, 1);
      assert.strictEqual(first[0].name, 'РеализацияТоваров');
      assert.strictEqual(parseCount, 1);

      // Mutate returned object
      first[0].name = 'MUTATED';
      first[0].children = [{ id: 'child', name: 'child', type: MetadataType.Attribute, properties: {} }];

      const second = await MetadataParser.parseTypeIndex(configPath, 'Documents');
      assert.strictEqual(parseCount, 1, 'second call must hit disk cache');
      assert.strictEqual(second.length, 1);
      assert.strictEqual(second[0].name, 'РеализацияТоваров', 'mutation of previous call result must not poison cache');
      assert.strictEqual(second[0].children?.length ?? 0, 0);
    } finally {
      MetadataParser.setTypeContentsCacheStoragePath(null);
      (FormatDetector.detect as unknown as typeof FormatDetector.detect) = originalDetect;
      (EdtParser.parseTypeIndex as unknown as typeof EdtParser.parseTypeIndex) = originalEdt;
      await fs.promises.rm(configPath, { recursive: true, force: true });
      await fs.promises.rm(storagePath, { recursive: true, force: true });
    }
  });

  test('parseTypeIndex works safely when storage path is null or folder is empty', async () => {
    const configPath = await fs.promises.mkdtemp(path.join(os.tmpdir(), '1cviewer-type-index-empty-cfg-'));
    await fs.promises.mkdir(path.join(configPath, 'Catalogs'), { recursive: true });
    ...
    try {
      ...
      assert.strictEqual(parseCount, 2, 'null storagePath operates uncached without error');
    } finally {
      (FormatDetector.detect as unknown as typeof FormatDetector.detect) = originalDetect;
      (DesignerParser.parseTypeIndex as unknown as typeof DesignerParser.parseTypeIndex) = originalDesigner;
      await fs.promises.rm(configPath, { recursive: true, force: true });
