...
suite('registerAgentCommands — debug commands registration', () => {
    ...
    test('все 15 debug-команд попадают в context.subscriptions', () => {
        ...
        // 12 CRUD + 2 type + 15 debug + 2 binding + 1 deploy + 4 agent deploy ops = 36 новых подписок
        assert.strictEqual(after - before, 36, `Ожидалось 36 подписок, получено ${after - before}`);
    });
