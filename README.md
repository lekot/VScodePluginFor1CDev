...
```
В этом workspace расширение VS Code «CDT 41» (1c-metadata-tree-vscode) поднимает HTTP-bridge с 53 командами для работы с метаданными, отладкой, формами (через Playwright) и СКД 1С. Прочитай файл .vscode/cdt-agent-bridge.json — там:
...
- docs — URL полного API reference (53 команды с примерами)
...
Используй этот API для задач 1С (CRUD метаданных, отладка BSL с точками останова, автоматизация enterprise-форм, CRUD СКД) вместо ручной правки XML или шагов в Конфигураторе. Если файла .vscode/cdt-agent-bridge.json нет — расширение не активировано, открой в workspace любой XML-файл метаданных 1С.
```
