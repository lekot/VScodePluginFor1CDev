...
А также: Визуальное редактирование типов реквизитов,  отображения ролей и их состава (без RLS и корня), фильтр по подсистемам, визуальный редактор форм (багованный прототип, идёт работа)

Чего нет: конструктор запроса (пользуйтесь внешними в режиме предприятия), СКД-редактора (я не готов), редактирование состава подсистем (когда-то будет)
...
- **Фильтр по подсистеме**: правый клик на узле подсистемы → «Filter by Subsystem» — показывает только объекты, принадлежащие данной подсистеме. Фильтр можно сбросить через контекстное меню или команду «Clear Subsystem Filter». (пока работает некорректно)
