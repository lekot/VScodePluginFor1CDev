...
Легковесное и быстрое расширение для Visual Studio Code для визуализации, разработки и редактирования конфигураций 1С:Предприятие (выгрузки Designer XML и EDT).

> ⚡ **Native TypeScript & No Java**: старт и отображение десятка конфигураций (включая масштабные решения уровня ERP и Управление холдингом) занимают около **20 секунд**, а потребление оперативной памяти составляет **менее 1 ГБ RAM** (в отличие от тяжеловесной среды 1C:EDT).

## Возможности

- 📁 **Дерево метаданных конфигурации 1С**: иерархическое отображение 40+ типов метаданных с ленивой загрузкой и мгновенным откликом. Поддержка выгрузок Designer XML и EDT.
- 🪄 **Встроенный Конструктор запросов 1С (Query Builder)**: визуальный редактор запросов SDBL на 11 вкладок (Таблицы и поля, Связи, Группировка, Условия WHERE/HAVING, Дополнительно, Объединения, Порядок, Индексы, Итоги, Пакет запросов, Текст запроса SDBL). Двусторонняя синхронизация текста и UI, конструктор выражений («карандаш» ✏️) с разворачиванием ссылочных полей через точку, параметры виртуальных таблиц регистров, поддержка соединений таблицы самой на себя (self-joins) и автогенерация BSL-кода (включая режим «с обработкой результата»).
- 🐞 **Нативная отладка BSL из VS Code (RDBG)**: отладочный адаптер на TypeScript (без сторонних Java-процессов) — точки останова (в т.ч. условные), пошаговое выполнение, стек вызовов, вычисление выражений, поддержка тонкого клиента и веб-сервера.
- 🔀 **Интеллектуальное сравнение и объединение конфигураций (Compare & Merge)**: семантическое сравнение метаданных по UUID/именам и BSL-модулей по процедурам и функциям (встроенный Compare View).
- 🎨 **Визуальные редакторы (Custom Editors & Webview)**:
  - **Редактор прав ролей**: интерактивная матрица прав доступа к объектам метаданных с сохранением в XML.
  - **Редактор типов**: примитивные, ссылочные и составные типы, квалификаторы длины и точности.
  - **Редакторы состава**: подсистем (с настройкой командного интерфейса), планов обмена, общих реквизитов, функциональных опций и критериев отбора.
  - **Редактор форм**: визуальное отображение структуры `Form.xml` с выбором темы оформления.
  - **Предпросмотр макетов**: визуализация табличных документов (MXL).
  - **Схемы XDTO-пакетов**: визуальный редактор схемы, экспорт в XSD, импорт из XSD, создание пакета из XSD, сравнение и объединение пакетов.
- 🚀 **Информационные базы, Deploy и запуск 1С**:
  - Привязка нескольких баз данных и массовая раскатка.
  - Инкрементальная раскатка измененных по Git файлов в базу через `ibcmd`.
  - Прямой запуск 1С:Предприятия (тонкий клиент) и Конфигуратора из расширения.
- 🔍 **Быстрый поиск и фильтрация**:
  - Сквозной поиск по имени, синонимам, комментариям и регулярным выражениям (RegEx).
  - Фильтрация по типам метаданных и контекстная фильтрация по подсистеме (ПКМ на узле подсистемы).
  - Позиционирование активного файла в дереве (`revealActiveFileInTree`).
- 🔒 **Хранилище конфигурации 1С (фаза 1)**: подключение к хранилищу, захват/освобождение объектов, помещение изменений и обновление конфигурации с безопасным хранением паролей в SecretStorage.
...
### Чего пока нет (в планах развития)

- Полноценный визуальный WYSIWYG-редактор управляемых форм с drag-and-drop элементов (сейчас доступен просмотр структуры и Form.xml, идёт работа над портированием визуального дизайнера).
- Интерактивный редактор макетов (сейчас поддерживается просмотр MXL).

Включайся в разработку — будет!
...
| CDT 41: Конструктор запроса... | Открыть визуальный конструктор запроса 1С (SDBL) для выделенного запроса или текущей строки |
| CDT 41: Конструктор запроса с обработкой результата... | Открыть конструктор с автоматической генерацией BSL-кода обхода результата и параметров |
| CDT 41: Редактор прав ролей | Открыть интерактивную матрицу прав ролей по метаданным |
| CDT 41: Редактировать состав подсистемы | Открыть webview редактирования состава выбранной подсистемы |
| CDT 41: Редактировать командный интерфейс подсистемы | Настройка видимости и порядка команд подсистемы |
| CDT 41: Редактировать состав плана обмена | Настройка состава и авторегистрации плана обмена |
| CDT 41: Редактировать состав общего реквизита | Настройка состава использования общего реквизита |
| CDT 41: Редактировать состав функциональной опции | Настройка состава объектов и реквизитов функциональной опции |
| CDT 41: Редактировать состав критерия отбора | Настройка состава критерия отбора |
| CDT 41: Редактировать схему XDTO-пакета | Открыть визуальный редактор структуры схемы XDTO |
| CDT 41: Импортировать XSD как новый XDTO-пакет | Создать новый XDTO-пакет из XSD-файла схемы |
...
- **Ctrl+Alt+Q** — открыть Конструктор запроса в редакторе BSL  
...
- Параметры функциональных опций (FunctionalOptionsParameters)
- Критерии отбора (FilterCriteria)
- Пакеты XDTO (XDTOPackages)
...
## Полезные ссылки и экосистема инструментов 1С

Рекомендуемые открытые инструменты сообщества для совместной работы с CDT 41 и AI-агентами:

- [bsl-language-server](https://github.com/1c-syntax/bsl-language-server) — протокол языкового сервера (LSP) и мощный статический анализатор (линтер) для 1С:Предприятие: глубокая проверка модулей всей конфигурации, контроль стандартов разработки и автоматические исправления (quick-fixes).
- [rlm-tools-bsl](https://github.com/Dach-Coin/rlm-tools-bsl) — токен-эффективный RLM-поиск по кодовой базе BSL, построение графа вызовов и быстрая навигация по процедурам/функциям. Рекомендуется подключать параллельно с CDT 41 при анализе, отладке и модификации кода в масштабных конфигурациях.
- [1c-mcp-toolkit](https://github.com/ROCTUP/1c-mcp-toolkit) — набор MCP-инструментов для сценариев, требующих прямого доступа к данным работающей информационной базы 1С (выполнение произвольных запросов, чтение Журнала регистрации, поиск ссылок на объекты с обязательной анонимизацией персональных данных).
