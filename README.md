...
- **CFE-проекты** (5 команд) — agent.cfe.{listProjects, getContext, validate, createProject, borrowObject}: создание расширения, устойчивое связывание с основной конфигурацией, проверка связей и идемпотентное заимствование поддержанных корневых объектов (Catalog, Document, Enum, CommonModule)
...
MCP-клиент находит endpoint через discovery-файл `.vscode/cdt-agent-bridge.json`: URL находится в `mcp.url`, Bearer token — в верхнеуровневом поле `token`. MCP публикует полный каталог **74/74**: metadata CRUD, CFE, support, EPF/ERF, bindings/deploy, types/subsystems, debug, forms, SKD и XDTO. Каждый `cdt_*` tool является строгой тонкой обёрткой над одноимённой Agent-командой; legacy `/command` остаётся доступен без изменений. Объекты адресуются через dot-path: `Catalog.Товары`, `Document.ПриходТовара.Attribute.Склад`.
